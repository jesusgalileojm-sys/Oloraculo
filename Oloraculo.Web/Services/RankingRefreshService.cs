...
namespace Oloraculo.Web.Services
{
    public class RankingRefreshService
    {
        ...
        public RankingRefreshService(HttpClient http, IWebHostEnvironment environment, IOptions<OloraculoConfig> options)
        {
            _http = http;
        ...
        public async Task<RankingRefreshReport> RefreshAsync(DateOnly? today = null, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                notes.Add($"FIFA rankings refreshed: {fifaRows} rows dated {fifaDate:yyyy-MM-dd}.");
            }
            catch (Exception ex)
            {
                errors.Add($"FIFA rankings refresh failed: {ex.Message}");
            }
            ...
            for (var offset = 0; offset <= lookbackDays; offset++)
            {
                ...
                try
                {
                    ...
                    await WriteAtomicAsync(Path.Combine(dataDirectory, OloraculoDataFiles.EloCsv), csv, ct);
                    eloRows = rows.Count;
                    eloDate = date;
                    updatedFiles.Add(OloraculoDataFiles.EloCsv);
                    notes.Add($"Elo rankings refreshed: {eloRows} rows dated {eloDate:yyyy-MM-dd}.");
                    break;
                }
                catch (Exception ex)
                {
            ...
            if (eloRows == 0)
            {
                ...
                errors.Add($"Elo rankings refresh failed after {lookbackDays + 1} date attempts. {sample}");
            }
        ...
        public static IReadOnlyList<FifaCsvRow> ParseFifaRankings(string raw)
        {
            ...
            if (rows.Count == 0)
                throw new InvalidOperationException("No FIFA ranking rows were parsed. The source format may have changed.");
        ...
        public static IReadOnlyList<EloCsvRow> ParseEloRankings(string html, DateOnly date, string sourceUrl)
        {
            ...
            if (headingIndex < 0)
                throw new InvalidOperationException("Could not find the Elo ratings heading. The source format may have changed.");
            ...
            if (rows.Count == 0)
                throw new InvalidOperationException("No Elo ranking rows were parsed. The source format may have changed.");
        ...
        private static DateOnly ParseFifaUpdatedDate(string raw)
        {
            ...
            if (!match.Success)
                throw new InvalidOperationException("Could not parse FIFA ranking update date.");
